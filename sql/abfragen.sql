...
--   BLOCK 2  — Zusatzabfragen für Aufgabe 2.5 (Pareto-Diagramm) und 2.6 (Kreativaufgabe)
--   BLOCK 4  — Strategische Geschäftsfälle (Fragen 4, 8, 9 + Verifikation + Kontrollrechnung 4.5)
...
-- #######################################################################
-- BLOCK 2 — Zusatzabfragen für Visualisierungen
-- #######################################################################

-- ---------------------------------------------------------------------
-- 2.5 Kundenbeitrag (Pareto): 20 Kundengruppen à 5%, sortiert nach Umsatz
--     NTILE(20) teilt die 3.000 Kunden nach absteigendem Umsatz in
--     20 gleich große Gruppen (je 150 Kunden = 5%). Grundlage für das
--     Pareto-Chart mit Balken (Umsatz je Gruppe) + Linie (kumulierter
--     Anteil), berechnet direkt in Tableau aus dieser Abfrage.
--     Export als CSV (block_2.5_pareto.csv) und separater Import in Tableau.
-- ---------------------------------------------------------------------
...
grouped AS (
    SELECT
        customer_id,
        total_spent,
        NTILE(20) OVER (ORDER BY total_spent DESC) AS group_number
    FROM customer_totals
)
SELECT
    group_number,
    (group_number - 1) * 5 + 1 AS pct_from,
    group_number * 5 AS pct_to,
    CAST((group_number - 1) * 5 + 1 AS TEXT) || '-' || CAST(group_number * 5 AS TEXT) || '%' AS group_label,
...
-- ---------------------------------------------------------------------
-- 2.6 Zusatzabfrage für Aufgabe 2.6 (freies Thema)
...
-- ---------------------------------------------------------------------
-- 4.5 (Frage 4 – Kontrollrechnung) LTV pro Akquisitionskanal ohne retournierte Bestellungen
--     Wiederholung von 4.1, diesmal ausschließlich mit Bestellungen
--     WHERE is_returned = 0. Dient als methodische Gegenprobe zu 4.1.
--     WICHTIG: Da nur nicht-retournierte Bestellungen berücksichtigt werden,
--     fallen Kunden, deren SÄMTLICHE Bestellungen retourniert wurden,
--     komplett aus dieser Berechnung heraus – die Kundenbasis ist daher
--     nicht identisch mit 4.1 (siehe bericht.md, Frage 4, methodischer Hinweis).
-- ---------------------------------------------------------------------
SELECT
    c.acquisition_chan AS acquisition_channel,
    COUNT(DISTINCT o.customer_id) AS customer_count,
    COUNT(o.order_id) AS orders_count,
    ROUND(AVG(customer_totals.total_spent), 2) AS avg_ltv_excl_returns
FROM shopsphere_orders o
JOIN shopsphere_customers c USING (customer_id)
JOIN (
    SELECT
    ...
    WHERE is_returned = 0
    GROUP BY customer_id
) customer_totals ON customer_totals.customer_id = o.customer_id
WHERE o.is_returned = 0
GROUP BY c.acquisition_chan
ORDER BY avg_ltv_excl_returns DESC;
