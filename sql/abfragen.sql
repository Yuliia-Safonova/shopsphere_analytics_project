-- =====================================================================
-- ShopSphere Analytics Project — Block 1: SQL-Datenaufbereitung
-- Autorin: Yuliia Safonova
-- Tool: sqliteonline.com (SQLite-Dialekt)
-- =====================================================================
-- Hinweis: Alle Abfragen basieren auf den 5 CSV-Tabellen (customers,
-- products, orders, order_items, marketing), die zuvor in
-- sqliteonline.com importiert wurden. Jede Abfrage bereitet einen
-- Datensatz für die spätere Visualisierung in Tableau (Block 2) vor.
-- =====================================================================


-- ---------------------------------------------------------------------
-- 1.1 Netto-Umsatz, Bestellanzahl und durchschnittlicher Bestellwert
--     pro REGION und JAHR
--     JOIN: orders <- customers (über customer_id)
-- ---------------------------------------------------------------------
SELECT
    c.region                          AS region,
...
-- ---------------------------------------------------------------------
-- 1.3 Pro Produktkategorie: Gesamtumsatz, durchschnittliche Marge
--     (margin_pct) und Retourenquote
--     JOIN: order_items <- products <- orders
--     Anmerkung: Die Retourenquote wird auf Positionsebene berechnet —
--     ist die zugehörige Bestellung retourniert (is_returned = 1),
--     zählt die Position als "retourniert".
-- ---------------------------------------------------------------------
SELECT
...
-- ---------------------------------------------------------------------
-- 1.4 Kunden, deren Gesamtausgaben über dem Durchschnitt aller Kunden
--     liegen (mittels Unterabfrage / CTE)
...
SELECT
    ...
    ROUND(
        SUM(total_spent) * 100.0 / (SELECT SUM(net_amount) FROM orders),
        2
